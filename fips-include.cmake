...
macro(bgfx_shaders)
    ...
    if ("${SHADERC_PATH}" STREQUAL "")
        if (FIPS_EMSCRIPTEN)
            if (FIPS_WINDOWS)
                file(GLOB_RECURSE SHADERC_PATHS ${FIPS_DEPLOY_DIR}/fips-bgfx/*shaderc.exe ${FIPS_DEPLOY_DIR}/*shaderc.exe)
            else()
                file(GLOB_RECURSE SHADERC_PATHS ${FIPS_DEPLOY_DIR}/fips-bgfx/*shaderc ${FIPS_DEPLOY_DIR}/*shaderc)
            endif()

            list(GET SHADERC_PATHS 0 SHADERC_PATH)
            if ("${SHADERC_PATH}" STREQUAL "SHADERC_PATH-NOTFOUND")
                message(FATAL_ERROR "Could not find shaderc executable.  Either fips build fips-bgfx (release mode is best) or set SHADERC_PATH" )
            else()
                message("Found shaderc: ${SHADERC_PATH}")
            endif()
        else()
            set(SHADERC_PATH "${FIPS_DEPLOY_DIR}/${FIPS_PROJECT_NAME}/${FIPS_CONFIG}/shaderc")
        endif()
    endif()
    ...
    foreach (cur_file ${_bs_FILES})
        ...
        fips_generate(TYPE BgfxShaderEmbedded FROM ${cur_file} HEADER ${_bs_OUTPUT}${out_file}.bin.h ARGS "{ shadercPath: \"${SHADERC_PATH}\" }" REQUIRES shaderc OUT_OF_SOURCE)
    endforeach()
endmacro()
